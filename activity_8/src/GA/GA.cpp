...
GA::GA(Instance *instance, GAType gaType, ld mutationRate, int popSize, int timeLimit, int target) :
 instance(instance), gaType(gaType), mutationRate(mutationRate), popSize(popSize), timeLimit(timeLimit), target(target), rng(random_device().operator()()) {}
...
Solution *GA::run() {
  ...
  while(true){
    generation++;
    nextGeneration();
    if(getTime() >= timeLimit || population[0]->getCost() >= target)
      break;
  }
